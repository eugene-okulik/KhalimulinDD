...
def test_create_posts_with_param(create_and_cleanup_post, name):
...
def test_create_single_post(create_and_cleanup_post):
    # Проверка созданного поста
    name = create_and_cleanup_post.response.json()["name"]
    create_and_cleanup_post.check_that_status_is_200()
    create_and_cleanup_post.check_response_name_is_correct(name)
